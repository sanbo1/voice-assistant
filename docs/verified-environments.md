...
訂正（2026-09-21）：上の「約 81 秒」は、別の読み込み処理と SD カードを取り合った状態での値だった。
競合のない状態で測り直したところ、次のとおり。

- 電源投入 13:57:55 → サービス開始 13:58:05（10 秒）
- サービス開始 → 起動完了「起動しました」13:59:14（**69 秒**）
- 起動完了からウェイクワード受付開始まで約 6 秒（無音・起動メッセージの読み上げ・お知らせ音のぶん）
- **電源投入からウェイクワードを受け付けるまで、合わせて約 85 秒**

補足（2026-09-21）：起動直後の読み上げが聞こえないことが 1 回あった（13:18 の起動）。原因は未確定。
そのとき並行して測定処理が SD カードを酷使していたため、通常の運用では起きない条件だった可能性がある。
次に起きたときに判断できるよう、起動時の出力先（デバイスと PipeWire のシンクの状態）を
会話ログに記録するようにした。13:57 の起動では出力先が SUSPENDED でも読み上げは聞こえており、
**「起動時に休止状態であること」自体は原因ではない**ことが分かっている。
